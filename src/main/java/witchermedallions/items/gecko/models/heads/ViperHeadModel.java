package witchermedallions.items.gecko.models.heads;

import net.minecraft.util.Identifier;
import software.bernie.geckolib3.model.AnimatedGeoModel;
import witchermedallions.items.gecko.item.heads.ViperHeadItem;
import witchermedallions.witcherMod;

public class ViperHeadModel extends AnimatedGeoModel<ViperHeadItem> {

    @Override
    public Identifier getModelResource(ViperHeadItem object) {
        return new Identifier(witcherMod.MODID, "geo/viper_head_medallion.geo.json");
    }

    @Override
    public Identifier getTextureResource(ViperHeadItem object) {
        return new Identifier(witcherMod.MODID, "textures/item/viper_off_medallion.png");
    }

    @Override
    public Identifier getAnimationResource(ViperHeadItem animatable) {
        return new Identifier(witcherMod.MODID, "animations/medallion_animation.animation.json");
    }
}
